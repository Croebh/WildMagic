...
def get_ttl_hash(seconds=14400):
...
class Search(commands.Cog):
    ...
    async def transcripts(self, ctx, search_type, user: disnake.Member):
        ...
        This caches the results for 4 hours, to reduce server/bot load.
        ...
        **Valid User Type:** Can either be an @ tag, or their user ID.
        ...
        for transcript in transcripts[search_type]:
        ...
        embed = disnake.Embed(title=search_type, description=f"Results for <@{user.id}>")
        for transcript in out:
            embed.add_field(name=transcript['ticket'],
                            value=f"{transcript['transcript']}\n"
                                  f"[Transcript Message]({transcript['message']})")
    ...
    async def get_transcripts(self, ttl_hash=None):
        ...
        transcripts = {"Character Submissions": [],
                       "Help and Support": [],
                       "Quest Request": []}
